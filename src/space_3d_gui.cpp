...
namespace {
enum { ID_ADD=100,ID_CLEAR,ID_PAUSE,ID_STEP,ID_COLLISION,ID_DROP,ID_FOUNTAIN,ID_IMPULSE,ID_SAVE,ID_LOAD,ID_BODY_RESET,ID_TIMER=1 };
...
LRESULT CALLBACK WndProc(HWND hwnd,UINT msg,WPARAM wp,LPARAM lp){
 ...
 case WM_KEYDOWN:if(wp==VK_SPACE&&!((lp>>30)&1))jump_queued=true;else if(wp=='P')engine.SetPaused(!engine.IsPaused());else if(wp==VK_DELETE&&selected){engine.RemoveBall(selected);selected=0;}else if(wp=='I')engine.ApplyImpulse(selected,{0,8,4});else if(wp=='R')body.Reset({0,engine.GetSettings().ground_y,0});UpdateInfo();return 0;
